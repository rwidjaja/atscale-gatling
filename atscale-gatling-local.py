...
def build_jar():
    ...
    print("Building executable JAR...")
    result = subprocess.run(["./mvnw", "clean", "package", "-DskipTests"], 
                          capture_output=True, text=True)
    if result.returncode != 0:
        print("Failed to build JAR:")
        print(result.stdout)
        print(result.stderr)
        sys.exit(1)
...
def run_local(selected_executor):
    """Run the selected executor using the pre-built JAR"""
    
    # Build the JAR if it doesn't exist
    jar_path = "target/atscale-gatling-1.0-SNAPSHOT.jar"
    ...
    try:
        ...
        java_cmd.extend([
            ...
            jar_path, 
            selected_executor,
            "working_dir/config/systems.properties"
        ])
        
        print(f"🚀 Running {selected_executor} with JAR...")
