...
public class OpenStepConcurrentSimulationExecutor extends ConcurrentSimulationExecutor<OpenStep> {
    ...
    private MavenTaskDto<OpenStep> createTaskForJsonFile(String cubeName, String catalogName, File queryFile, 
        ...
        try {
            ...
            task.setRunId("ConcurrentOpenRun-" + cubeName.replace(" ", "") + "-" + protocol + "-JSON");
    ...
    private MavenTaskDto<OpenStep> createTaskForCsvFile(String cubeName, String catalogName, File csvFile, 
        ...
        try {
            ...
            // Set other required properties from the original example
            task.setMavenCommand("gatling:test");
            task.setRunId("ConcurrentOpenRun-" + cubeName.replace(" ", "") + "-" + protocol + "-CSV");
            task.setRunLogFileName(cubeName.replace(" ", "_") + "_" + protocol + "_concurrent_open_csv.log");
            task.setLoggingAsAppend(false);
            ...
            logger.info("Created concurrent open step CSV task for cube: " + cubeName + 
                       " (catalog: " + catalogName + ")" +
                       ", protocol: " + protocol + 
