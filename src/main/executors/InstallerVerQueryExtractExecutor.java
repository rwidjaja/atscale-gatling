...
public class InstallerVerQueryExtractExecutor {
    ...
    protected void execute() {
        ...
        for (Map.Entry<String, CubeConfig> cubeEntry : cubeConfigs.entrySet()) {
            ...
            // Run JDBC extraction with query_language = 'pgsql'
            cacheJdbcQueries(config.cubeName, baseQuery);

            // Run XMLA extraction with query_language = 'analysis'
    ...
    private String loadBaseQuery() {
        try {
            java.io.File queryFile = new java.io.File(WORKING_DIR,
                    CONFIG_DIR + java.io.File.separator + BASE_QUERY_FILE);
        ...
        for (String propertyName : props.stringPropertyNames()) {
            if (propertyName.startsWith("atscale.") && propertyName.contains(".xmla.cube")) {
                String cubeIdentifier = propertyName.substring("atscale.".length(),
                        propertyName.indexOf(".xmla.cube"));
    ...
    private void cacheJdbcQueries(String cubeName, String baseQuery) {
        ...
        // Pass "pgsql" as the query language for JDBC
        queryHistoryFileUtil.cacheJdbcQueries(cubeName, baseQuery, "pgsql", cubeName);
    }
    ...
    private java.util.Properties getProperties() {
        try {
            ...
            java.io.File file = new java.io.File(WORKING_DIR + java.io.File.separator + CONFIG_DIR
                    + java.io.File.separator + "systems.properties");
            if (file.exists()) {
