...
public class ClosedStepConcurrentSimulationExecutor extends ConcurrentSimulationExecutor<ClosedStep> {
    ...
    protected List<MavenTaskDto<ClosedStep>> getSimulationTasks() {
        ...
        return tasks; // Removed withAdditionalProperties call since we're handling properties directly
    }
    ...
    private MavenTaskDto<ClosedStep> createTaskForJsonFile(String cubeName, String catalogName, File queryFile, 
        ...
        try {
            ...
            task.setRunId("ConcurrentClosedRun-" + cubeName.replace(" ", "") + "-" + protocol + "-JSON");
            task.setRunLogFileName(cubeName.replace(" ", "_") + "_" + protocol + "_concurrent_closed.log");
            task.setLoggingAsAppend(false);
            
            // Build additional properties as a Map
            ...
            additionalProperties.put("catalog", catalogName); // For XMLA
            ...
            logger.info("Created concurrent closed step task for cube: " + cubeName + 
                       " (catalog: " + catalogName + ")" +
                       ", protocol: " + protocol + 
    ...
    private MavenTaskDto<ClosedStep> createTaskForCsvFile(String cubeName, String catalogName, File csvFile, 
        ...
        try {
            ...
            // Set other required properties
            task.setMavenCommand("gatling:test");
            task.setRunId("ConcurrentClosedRun-" + cubeName.replace(" ", "") + "-" + protocol + "-CSV");
            task.setRunLogFileName(cubeName.replace(" ", "_") + "_" + protocol + "_concurrent_closed_csv.log");
            task.setLoggingAsAppend(false);
            ...
            logger.info("Created concurrent closed step CSV task for cube: " + cubeName + 
                       " (catalog: " + catalogName + ")" +
                       ", protocol: " + protocol + 
